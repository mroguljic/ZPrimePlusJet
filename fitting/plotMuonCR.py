...
def main(options, args):
    fml = rt.TFile.Open(options.input+'/mlfit.root','read')


    #bkgs = ['qcd','tqq','stqq','zqq','wqq','wlnu','vvqq','zll','total_background']
    #bkgs = ['qcd','tqq','stqq','zqq','wqq','wlnu','vvqq','total_background']
    bkgs = ['tqq','stqq','wlnu'] #,'total_background']
    #sigs = ['hqq125','tthqq125','whqq125','zhqq125']
    sigs = []#'zqq'+str(options.mass)]
    data = ['data']
    boxes = ['muonCR_fail_muonCR','muonCR_pass_muonCR']

    shapes = {}
    for box in boxes:
        for proc in (bkgs+sigs+data):
            print 'shapes_%s/%s/%s'%(options.fit,box,proc)
    ...
    for box in boxes:
        ...
        for proc in bkgs:
            if i0 == 0:
                htot = shapes['%s_%s'%(box,proc)].Clone()
            else:
                htot.Add( shapes['%s_%s'%(box,proc)].Clone() )
            i0 += 1
...
if __name__ == '__main__':
    ...
    parser.add_option('--lumi', dest='lumi', type=float, default = 41.1,help='lumi in 1/fb ', metavar='lumi')
    parser.add_option('-i','--input', dest='input', default = './',help='directory with data', metavar='idir')
    ...
    parser.add_option('--jet', dest='jet', default ='AK8',help='jet')
