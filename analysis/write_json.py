...
def main(options,args):
    if not options.printOnly:
        outf = open(os.path.expandvars("data/samplefiles.json"),"w")
        print "Writing to ", os.path.expandvars("data/samplefiles.json")
    finaljson = {}
    finaljson['zqq2017'] = expandPath(zqq_files.get2017files()) 
    if not options.printOnly:
